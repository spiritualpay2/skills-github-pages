document.addEventListener('DOMContentLoaded', () => {
    ...
    async function displayPage(index) {
        ...
        if (!zip || index < 0 || index >= sortedImageFiles.length || isTransitioning) {
             ...
             if (isTransitioning) { setTimeout(() => { isTransitioning = false; }, 100); }
             return;
         }
         ...
         // newImage.style.opacity = 0; // DEBUG: Don't set opacity initially

         const oldImage = currentImageElement;
         currentImageElement = newImage;

         console.log(`displayPage: Adding new image element for index ${targetPageIndex} to DOM.`); // LOG 21
         imageContainer.appendChild(newImage);

        // --- 3. Load Image Source & Handle Results (SIMPLIFIED) ---
         try {
             await new Promise((resolve, reject) => {
                 newImage.onload = () => {
                     console.log(`displayPage: newImage.onload triggered for index ${targetPageIndex}.`); // LOG 22 Simplified
                     // DEBUG: DO NOT apply fit or transitions here initially
                     resolve();
                 };
              ...
              // --- 4. Image Loaded (SIMPLIFIED) ---
              console.log(`displayPage: Image ${targetPageIndex + 1} promise resolved.`); // LOG 25 Simplified

              // DEBUG: Directly make sure it's visible and flag transition done for first load
              newImage.style.opacity = 1; // Ensure visible if CSS didn't default
              isTransitioning = false; // Allow next actions
              preloadAdjacentPages(targetPageIndex);
              updateUIState();
              console.log(`displayPage: Simplified display complete for index ${targetPageIndex}.`); // LOG NEW

              // Handle cleanup of old image if necessary (though likely only relevant on subsequent loads)
              // This simplified logic might briefly show two images if navigating fast, but ok for debug
              if (oldImage) {
                  if (imageContainer.contains(oldImage)) {
                      imageContainer.removeChild(oldImage);
                      console.log("displayPage: Removed old image from DOM (Simplified path).");
                  }
              }
    ...
    function applyZoom(centerPoint = null) {
         if (!currentImageElement || !currentImageElement.naturalWidth || isFitMode) return; // Don't apply if fitting
         ...
         img.style.border = ''; // Remove debug border when zooming
    ...
    function zoomIn(e = null) {
         ...
         isFitMode = false; // Exit fit mode on zoom in
     ...
     function zoomOut(e = null) {
         if (isFitMode) return;
         isFitMode = false; // Ensure we aren't fitting
         currentZoom = Math.max(currentZoom - ZOOM_STEP, MIN_ZOOM);
         const centerPoint = e ? { x: e.clientX, y: e.clientY } : null;
         applyZoom(centerPoint);
         // Check if we should revert to fit mode
     ...
     function zoomFit(triggerControlShow = true) {
         ...
         img.style.border = '2px dashed red'; // Re-apply debug border on fit
    ...
    function handlePanStart(e) {
        ...
        // Can only pan if NOT in fit mode AND image is scrollable
